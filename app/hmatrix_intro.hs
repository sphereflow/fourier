...
import Data.List (sort, nub, group, sortBy)
...
main = do
   ...
   writeImageToBMP "111.bmp" (last drepa)
   mirrage <- imageNewFromFile "111.bmp"
...
data CFunc = CFDCT | CFHAAR
instance Eq CFunc where
   (==) CFDCT CFDCT = True
   (==) CFHAAR CFHAAR = True
   (==) _ _ = False
...
repaTo3Planes t =
   (repaToMatrix ((getP 1) $ unzipped),
   repaToMatrix ((getP 2) $ unzipped),
   repaToMatrix ((getP 3) $ unzipped))
   where unzipped = RV.unzip3 t

-- fill matrix to multiple of block size
fillMatrix :: (Element a, Show a) => Int -> Matrix a -> Matrix a
fillMatrix bs m
   ...
   where
      r = M.fromLists [ take margin_right $ repeat (m @@> (i, w - 1)) | i <- [0 .. (h - 1)] ]
      b = trans $ M.fromLists [ take margin_bottom $ repeat (m @@> (h - 1, i)) | i <- [0 .. (w -1)] ]
      br = (margin_bottom >< margin_right) $ repeat (m @@> (h - 1, w - 1))
...
-- bs = block size
tileMatrix :: (Element a, Show a) => Int -> Matrix a -> [Matrix a]
...
getP :: Int -> (Array U DIM2 a, Array U DIM2 a, Array U DIM2 a) -> Array U DIM2 a
getP 1 ( p , _ , _ ) = p
getP 2 ( _ , p , _ ) = p
getP 3 ( _ , _ , p ) = p
getP _ ( p , _ , _ ) = p -- return red plane by default
...
transMat :: Int -> (Int -> (Int, Int) -> Double) -> Matrix Double
transMat n coeffFunc = buildMatrix n n (coeffFunc n)

getCoeffFunc :: CFunc -> Int -> (Int, Int) -> Double
getCoeffFunc CFDCT = dctCoeff
getCoeffFunc CFHAAR = haarCoeff

dctCoeff :: Int -> (Int, Int) -> Double
dctCoeff n (i, j) = c0 i * sqrt (2.0 / fromIntegral n) * cos (fromIntegral ((2 * j + 1) * i) * pi / fromIntegral (2 * n))

haarCoeff :: Int -> (Int, Int) -> Double
haarCoeff n (i, j) = hw (fromIntegral i) ((fromIntegral j) / (fromIntegral n)) (fromIntegral n)
...
meanSquareDistance m1 m2 = (sumElements $ mul diffm diffm) / (fromIntegral $ size * size)
   where
      diffm = sub m1 m2
      size = rows m1

meanAbsDistance :: Matrix Double -> Matrix Double -> Double
meanAbsDistance m1 m2 = (sumElements $ mapMatrix abs diffm) / (fromIntegral $ size * size)
   where
      diffm = sub m1 m2
      size = rows m1
